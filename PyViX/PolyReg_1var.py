...
def gen_data(num_samples = 100):
    ...
    for i in range(num_samples):
        ...
        data_X.append(x)
        data_Y.append(6 * x * x + np.random.normal(0, 0.5))
...
if __name__ == "__main__":
    ...
    mu_0, sigma_0 = Variable(), Variable(init = 3, range=[0.1,5])
    a0 = Uniform(0, 20, name = 'a0',learnable_params=[mu_0, sigma_0])
    data = Data(data_X, name = 'data')
    Y_obs_data = Data(data_Y, name = 'Y_obs_data')
    Y = Observed(dist = Normal(a0 * data * data, 1, name = 'Y'), data = Y_obs_data, name = 'Y')
    M = Model([Y, a0, data], params = parameters)
    
    gen_code(M, file_name = "poly_1v", type = "float")
