...
#define T fpm::fixed<std::int32_t, std::int64_t, 15>
#define SIGMA_LB 0.1
#define SIGMA_UB 5
std::mt19937 gen(0);
...
auto Infer(Obs_ty &N, prior_T& T_prior, guide_T& T_var,mu_learn_ty& mu_var, std_learn_ty& sigma_var,
                 T learning_rate, int  NUM_SAMPLES, int NUM_ITERS = 1000)
{
    ...
    for(int c_iter = 0;c_iter < NUM_ITERS; c_iter++)
    {
        ...
        //[ARD:REMOVE_END]
        T_var.mu(T_var.mu.value() + learning_rate*final_grad_mean);
        T_var.stddev(grad_clipping<T>(T_var.stddev.value() + learning_rate*final_grad_sigma, SIGMA_LB, SIGMA_UB));
...
int main()
{
    ...
    std::array<std::array<T,3>, 50> X_true;
    ...
    for(int time_step = 0; time_step < 100; time_step++)
    {
        ...
        if(time_step % 5 == 0) // optimize this for fixedpoint
        {
            call_gps = true;
        }
        else
