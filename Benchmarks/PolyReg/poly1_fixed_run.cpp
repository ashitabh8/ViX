...
int main(int argc, char** argv)//[ARD:SETUP_DEF]
{
    ...
    const int NUM_DATA_SAMPLES = 225;
    ...
    for(int c_iter = 0; c_iter < NUM_ITERS; c_iter++)
    {
        ...
        for (int i = 0; i < NUM_SAMPLES; i++)
        {
            ...
            second_term = current_score + T::from_raw_value(a0_prior.log_pdf().value().raw_value() >>BITSHIFT) - T::from_raw_value(a0.log_pdf().value().raw_value() >> BITSHIFT);
        ...
        T mean_over_samples_del_elbo_mean = grad_clipping<T>(T::from_raw_value(current_del_elbo_mu_a0.raw_value()>> BITSHIFT_SAMPLES), LB, UB); // Remove these extra variables when doing performance testing
        T mean_over_samples_del_elbo_sigma =grad_clipping<T>(T::from_raw_value(current_del_elbo_sigma_a0.raw_value()>> BITSHIFT_SAMPLES), LB, UB);
        // if (c_iter > 4)
        // {
        ...
        // std::cout << ",  Iteration: " << c_iter << "-> mean: " << a0_mean.value() << ", sigma: " << a0_sigma.value() << ", Loss: " <<total_elbo/NUM_SAMPLES <<std::endl;
    }
