...
#define T fpm::fixed<std::int32_t, std::int64_t, [PY:FRAC]> // 8 is the value 
#define SIGMA_LB 0.1
#define SIGMA_UB 5
...
int main(int argc, char** argv) //[ARD:SETUP_DEF]
{
    ...
    std::array<float, [PY:NUM_EXP]> means;

    //[ARD:REMOVE_BEGIN]
    for(int curr_exp = 0; curr_exp < [PY:NUM_EXP]; curr_exp++)
    {
    ...
    //[ARD:REMOVE_BEGIN]
    std::cout << std::abs(90.3 - float(mu_var.value()))/95.4 <<", ";
    means[curr_exp] = (float) mu_var.value();
    //[ARD:REMOVE_END]
    ...
    // Serial.println("95.8," + String((float)max_mean,5) + "," + String((float)rmse_current,5) +"," + String((float)curr_total_time) +"," + String((float)NUM_ITERS) +"," + String((float)NUM_SAMPLES) +"," + String((float)1));
    //[ARD:UNCOMMENT_END]
