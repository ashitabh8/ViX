...
Serial.begin(9600); while(!Serial){;}
    ...
    unsigned long curr_total_time = end_time - start_time;
     Serial.println("Total time fixed: " + String((float) curr_total_time));
    ...
    float rmse_i = rmse(yobs_positions_float, predicted_positions_float,0);
    float geomean_i = geomean(yobs_positions_float, predicted_positions_float,0);
    rmse_all_experiments[curr_exp] = rmse_i;
    geomean_all_experiments[curr_exp] = geomean_i;
    Serial.println("Geomean: " + String((float) geomean_i,6));
