...
Serial.begin(9600); while(!Serial){;}
    ...
    for(int curr_exp=0; curr_exp <num_experiments; curr_exp++)
    {
        ...
        // std::array<T,NUM_DATA_POINTS> error;
        // for(int j = 0; j < predicted_positions.size(); j++)
        // {
        //     error[j] = (predicted_positions[j] - true_positions[j])/true_positions[j];
        // }
        ...
        // }
         ...
         Serial.println("Total time float: " + String((float) curr_total_time));
         total_time += curr_total_time;
     // Serial.println("Average Total time float: " + String((float)total_time/num_experiments));
     
        float curr_rmse = rmse(yobs_positions, predicted_positions, 10);
        float curr_geomean = geomean(yobs_positions, predicted_positions, 10);
        rmse_all_experiments[curr_exp] = curr_rmse;
        geomean_all_experiments[curr_exp] = curr_geomean;

        // std::cout << curr_rmse << "," << curr_geomean << "," << NUM_ITERS << "," << NUM_DATA_POINTS << "," << NUM_SAMPLES << std::endl;

        // for(int i = 0; i < predicted_positions.size(); i++)
        // {
        //     std::cout << i << "," << yobs_positions[i] << "," << predicted_positions[i] << std::endl;
        // }
        //break;
