...
int main()
{
    ...
    std::ofstream result_file("Linreg_6_x_2_result_float.csv");
        ...
        for (int i = 0; i < NUM_SAMPLES; i++)
        {
            ...
            for(int data_k = 0; data_k < NUM_DATA_SAMPLES; data_k++)
            {
                ...
                Y_container(y_test[data_k]);
                    ...
                    current_score += Y.log_pdf().value()/ std::pow(2, BITSHIFT);
            }

            second_term = current_score + (a0_prior.log_pdf().value() + a1_prior.log_pdf().value() - a0.log_pdf().value() - a1.log_pdf().value())/std::pow(2, BITSHIFT);
