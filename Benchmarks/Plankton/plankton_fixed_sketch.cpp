...
int main(int argc, char ** argv)//[ARD:SETUP_DEF]
{
    ...
    const T learning_rate = (T) [PY:LEARNING_RATE];
    ...
    for(int c_iter = 0; c_iter < NUM_ITERS; c_iter++)
    {
        ...
        //[ARD:REMOVE_BEGIN]
        result_file << c_iter<<", " << total_elbo/NUM_SAMPLES_T << ", " << mu_plankton_var.value() << ", " << sigma_plankton_var.value() << ", "<< final_grad_mean<< std::endl;
        //[ARD:REMOVE_END]
