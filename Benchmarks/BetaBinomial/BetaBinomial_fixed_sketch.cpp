...
int main(int argc, char** argv)//[ARD:SETUP_DEF]
{
   ...
   //[ARD:UNCOMMENT_BEGIN]
    ...
    // std::array<float, 1> y_true{[PY:DATA]};
    ...
    Constant<T> N([PY:N]), data([PY:DATA]);
    ...
    auto p = normal_dist<T>(mu_p, sigma_p, p_sample);
        ...
        const int NUM_ITERS = [PY:NUM_ITERS];
        ...
        for(int c_iter = 0;c_iter < NUM_ITERS; c_iter ++)
        {
            ...
            //[ARD:REMOVE_BEGIN]
            result_file << c_iter << "," << total_elbo/NUM_SAMPLES_T << "," << mu_p.value()/T{100} << "," << sigma_p.value() << ", " << final_grad_mean <<std::endl;
            //[ARD:REMOVE_END]
    ...
    // Serial.println("[PY:DATA]," + String((float)max_mean,5) + "," + String((float)rmse_current,5) +"," + String((float)curr_total_time) +"," + String((float)NUM_ITERS) +"," + String((float)NUM_SAMPLES) +"," + String((float)1));
    //[ARD:UNCOMMENT_END]
